  ...
  return (
    <div>
      ...
      <div style={{ height: 6, borderRadius: 3, background: "rgba(255,255,255,0.06)", overflow: "hidden", position: "relative" }}>
        <div style={{ position: "absolute", top: 0, left: 0, height: "100%", borderRadius: 3, width: `${timeW}%`, background: "#ff4757", opacity: 0.6, transition: "width 0.5s ease" }} />
        <div style={{ position: "absolute", top: 0, left: 0, height: "100%", borderRadius: 3, width: `${taskW}%`, background: "#00b894", transition: "width 0.5s ease" }} />
      </div>
    </div>
  );
}
...
export default function SprintTracker({ sprint, onCheckedChange, onStartDateChange, onReset }) {
  const { months, tracks, checked, startDate, title, subtitle } = sprint;
  ...
  function getOverallTaskProgress() {
    ...
    return keys.filter((k) => checked[k]).length / keys.length;
  }
  ...
  function getTrackProgress(trackKey) {
    ...
    months.forEach((m) => {
      (m.tracks[trackKey] || []).forEach((_, idx) => {
        total++;
        if (checked[`${m.month}-${trackKey}-${idx}`]) done++;
      });
    });
    return total > 0 ? done / total : 0;
  }
      ...
      <div style={{ background: "linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%)", borderBottom: "2px solid #c9a84c", padding: "28px 24px 24px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
            <div>
              <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 26, fontWeight: 700, color: "#c9a84c", lineHeight: 1.2 }}>{title}</div>
              <div style={{ fontSize: 12, color: "#8a8a9a", marginTop: 5, maxWidth: 500 }}>{subtitle}</div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setShowSettings(!showSettings)} onMouseEnter={() => setBtnHover("settings")} onMouseLeave={() => setBtnHover("")} style={{ background: btnHover === "settings" ? "rgba(201,168,76,0.28)" : "rgba(201,168,76,0.15)", border: "1px solid rgba(201,168,76,0.3)", color: "#c9a84c", padding: "6px 14px", borderRadius: 6, fontSize: 12, cursor: "pointer", transition: "all 0.2s", fontFamily: "inherit" }}>⚙️ Settings</button>
              <button onClick={() => setShowResetConfirm(true)} onMouseEnter={() => setBtnHover("reset")} onMouseLeave={() => setBtnHover("")} style={{ background: btnHover === "reset" ? "rgba(255,71,87,0.22)" : "rgba(255,71,87,0.1)", border: "1px solid rgba(255,71,87,0.35)", color: "#ff4757", padding: "6px 14px", borderRadius: 6, fontSize: 12, cursor: "pointer", transition: "all 0.2s", fontFamily: "inherit" }}>↺ Reset</button>
            </div>
          </div>

          {showSettings && (
            <div style={{ marginTop: 16, padding: 16, background: "rgba(0,0,0,0.3)", borderRadius: 8, border: "1px solid rgba(201,168,76,0.2)" }}>
              <label style={{ fontSize: 12, color: "#8a8a9a", display: "block", marginBottom: 6 }}>Sprint start date (sets the time baseline for progress bars)</label>
          ...
          <div style={{ marginTop: 20, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <ProgressBar label="Time elapsed" pct={overallTime} color="#ff4757" />
            <ProgressBar label="Tasks completed" pct={overallTask} color="#00b894" />
          </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 24px 60px" }}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 20, flexWrap: "nowrap", overflowX: "auto", WebkitOverflowScrolling: "touch", scrollbarWidth: "none", msOverflowStyle: "none" }}>
          {months.map((m) => {
            ...
            return (
              <button key={m.month} onClick={() => toggleMonth(m.month)} style={{ flex: "0 0 auto", minWidth: 80, padding: "10px 8px", borderRadius: 8, cursor: "pointer", background: isExpanded ? "rgba(201,168,76,0.15)" : "rgba(255,255,255,0.03)", border: isExpanded ? "1px solid rgba(201,168,76,0.4)" : "1px solid rgba(255,255,255,0.06)", transition: "all 0.2s", fontFamily: "inherit" }}>
                ...
                <div style={{ marginTop: 6, height: 3, borderRadius: 2, background: "rgba(255,255,255,0.08)", overflow: "hidden" }}>
                  <div style={{
                    ...
                    background: taskPct >= getMonthTimeProgress(m.month) ? "#00b894" : `linear-gradient(90deg,#00b894 ${(taskPct / Math.max(getMonthTimeProgress(m.month), 0.01)) * 100}%,#ff4757 ${(taskPct / Math.max(getMonthTimeProgress(m.month), 0.01)) * 100}%)`,
        ...
        {months.map((m) => {
          ...
          const taskPct = getMonthTaskProgress(m.month);
          return (
            <div key={m.month} style={{ marginBottom: 12, borderRadius: 12, overflow: "hidden", background: isExpanded ? "rgba(255,255,255,0.04)" : "rgba(255,255,255,0.02)", border: isExpanded ? "1px solid rgba(201,168,76,0.25)" : "1px solid rgba(255,255,255,0.05)", transition: "all 0.3s" }}>
              <div onClick={() => toggleMonth(m.month)} style={{ padding: "16px 20px", cursor: "pointer", display: "flex", alignItems: "center", gap: 16 }}>
                <div style={{ width: 38, height: 38, borderRadius: "50%", flexShrink: 0, background: taskPct === 1 ? "#00b894" : "linear-gradient(135deg,#1a1a2e,#0f3460)", border: taskPct === 1 ? "2px solid #00d2a0" : "2px solid #c9a84c", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 700, color: taskPct === 1 ? "#fff" : "#c9a84c" }}>
                  {taskPct === 1 ? "✓" : `M${m.month}`}
                </div>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ marginTop: 8 }}><DualProgressBar timePct={timePct} taskPct={taskPct} /></div>
                </div>
              ...
              {isExpanded && (
                <div style={{ padding: "0 20px 20px" }}>
                  {Object.entries(m.tracks).map(([trackKey, items]) => {
                    ...
                    return (
                      <div key={trackKey} style={{ marginTop: 12, padding: 14, borderRadius: 8, background: `${meta.color}08`, border: `1px solid ${meta.color}18` }}>
                        ...
                        {items.map((item, idx) => {
                          const key = `${m.month}-${trackKey}-${idx}`;
                  ...
                  <div style={{ marginTop: 14, padding: "10px 14px", borderRadius: 6, background: "rgba(201,168,76,0.08)", border: "1px solid rgba(201,168,76,0.2)" }}>
                    <div style={{ fontSize: 10, color: "#c9a84c", fontWeight: 600, textTransform: "uppercase", letterSpacing: 0.5 }}>🏁 End of Month {m.month}</div>
